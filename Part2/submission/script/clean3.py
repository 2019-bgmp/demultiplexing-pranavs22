...
def de_multiplex(R1,R2,R3,R4,index_list):
    ...
    #opebn all files
    with gzip.open(R1) as r1, gzip.open(R2) as r2 , gzip.open(R3) as r3 , gzip.open(R4) as r4, open(swapped_r1,"a") as s1, open(swapped_r2,"a") as s2,open(Undetermined_r1,"w") as u1 ,open(Undetermined_r2,"w") as u2 :        
        while True:
            ...
            if header_r1=='':
                ...
                seq_r1=r1.readline().decode('utf-8').strip()
                q_head_r1=r1.readline().decode('utf-8').strip()
                q_seq_r1=r1.readline().decode('utf-8').strip()
        ...
        print("Total Number of records processed:", total_records)
        print("Number of dual matched pairs:",dual_matched)
        print("Number of index hopped pairs:",swapped)
        print("Number of undetermined read pairs:",undetermined)
        print("Percentage of dual matched:",str((dual_matched/total_records)*100))
        print("Percentage of index hopped read pairs:",str((swapped/total_records)*100))
        print("Percentage of undetermined reads:",str((undetermined/total_records)*100))
        print("Information about indexes and number of reads for each index")
        print("Index \t Number of reads \t Percentage")
         
        for k,v in index_count.items():
            print(k ,"\t",v,"\t",((v/total_records)*100))
            graph_index.append(k)             
            graph_count.append(v)
        plt.xlabel="Index"
        plt.ylabel="Count of reads"
        plt.title="Index and Read Count"
        plt.bar(graph_index,graph_count,color="green")
        plt.xticks(graph_index)
        plt.savefig(out + "indexes.png")
